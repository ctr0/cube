...
public final class CopyOfLexer {
	...
	public CopyOfLexer(InputStreamReader stream) throws IOException {
		...
		for (;;) {
			...
			switch (buffer.chars[end]) {
				...
				case '>': end++; 
					...
					return Id._GT;
				// Potential keywords 
				case 'a': return keywordOrIdentifier(Id.ABSTRACT, /*a*/"bstract", 7);
				...
				case 'd': return keywordOrIdentifier(Id._DOUBLE, /*d*/"ouble", 5);
...
//					case 'i': return keywordOrIdentifier(Id._FINAL, /*fi*/"nal", 3);
				...
				case 't': if (match(/*t*/"ype", 3)) switch (buffer.chars[++end]) {
				...
				case 'v': switch (buffer.chars[++end]) {
					...
					case 'o': return keywordOrIdentifier(Id.VOLATILE, /*vo*/"latile", 6);
					default:  return identifier();
				}
...
//					end++;
	...
	public boolean match(int[] matches) {
		return match(matches, begin) | match(matches, end);
	}
	
	private boolean match(int[] matches, int begin) {
		int scan = begin;
		for (;;) {
