...
public class HxxBuilder extends Preprocessor {
	...
	private void parseAccessModifier(int[] modifiers, long recovery) {
		modifiers[0] = getIdAndConsume() | (modifiers[0] & ~(Id.PUBLIC|Id.PROTECTED|Id.PRIVATE));
	...
	private void parseUsingDir(ScopeVisitor visitor, long recovery) {
		...
		if (token != null) {
			UsingDecl dir = new UsingDecl(token, getAnnotations());
			...
			if (token.isComplex()) { 
				...
				for (int i = 0; i < tokens.length -1; i++) {
					visitor = visitor.visitNamespaceDecl(
						new NsDecl(tokens[i], null)
					);
			}	}
			return visitor.visitNamespaceDecl(
				new NsDecl(token, getAnnotations())
			);
		}
		return null;
	}
	...
	private void parseRecord(ScopeVisitor visitor, int[] modifiers, long recovery) {
		...
		if (token != null && recover(r, recovery)) {
			RecordDecl decl = new RecordDecl(modifiers[0], token, getAnnotations());
			modifiers[0] = 0;
			Template d = null;
			DO: do switch (id()) {
				case Id.LTLT:
...
//				}
	...
	private void parseRecordDtor(ScopeVisitor visitor, int[] modifiers, long recovery) {
		...
		IToken token = parseIdentifier(recovery);
		if (token != null) {
			MethodDecl decl = new MethodDecl(modifiers[0], token, getAnnotations());
		...
		if (token != null) {
			...
			DO: do switch (id()) {
				...
				default: // Handle all operators 
					...
					//$FALL-THROUGH$
				case Id.SCOLON:
					VarDecl decl = new VarDecl(type, token, getAnnotations());
					if (expr != null) expr.accept(decl);
