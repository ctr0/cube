...
public class Builder extends Parser{
	...
	private static final long $RECORD_SAFE = 	Id.$$MODS|Id.$$RECO|Id.$$ID|Id.$$TYPE|Id.$ATSIGN;
	...
	public Builder(CodeSource source) throws FileNotFoundException, IOException {
		super(source);
	}
...
//				}
	...
	private Template parseGenericDecl(long recovery) {
	...
	private void parseGenericParam(Template[] ref, long recovery) {
		...
		if (type != null) {
			...
			do switch (id()) {
				case Id.COLON:
					...
					if (ref[0] == null) ref[0] = new Template();
...
//					}
	...
	private void parseType(Type type, long recovery) {
		...
		case Id.LTLT: 
			...
			//$FALL-THROUGH$
		case Id.LT:
			Template d = parseGenericDecl(recovery|Id.$LT_LTLT);
	...
	private Expression buildExpr(Expression/*?*/ e1, Expression e2, IExpr o) {
		if (e1 != null) {
			if (Id.Op.compare(e1.getId(), e2.getId()) >= 0) { // FIXME > OR >= ???????
				e1.accept(e2);
				if (o != null) 
					o.accept(e1);
				return e2;
...
//					parseForStat(visitor, recovery);
			...
			case Id.ASSERT:	parseInstrExprStat(
				visitor, getAndConsumeToken(),
				parseExpression(recovery|Id.$SCOLON), recovery);
				return;
...
//		if (expect(Id.$OPEN_PAR, Id.$CLOSE_PAR|Id.$OPEN_KEY|$BLOCK_SAFE, "MSG")) {
//			consume(); // `('
//			InstrStat instr = new InstrStat(token);
//			// Parse the first expression
//			parseVarDeclOrExpr(instr, null, $BLOCK_SAFE);
//			// Parse next expressions
//			if (!recover(Id.$CLOSE_PAR, Id.$SCOLON | Id.$OPEN_KEY | $BLOCK_SAFE)) {
//				expect(Id.$SCOLON, $BLOCK_SAFE, Report._1_ERR_MISS_STMT_SCOLON);
//				do {
//					IExpr expr = parseExpression(
//						Id.$SCOLON | Id.$CLOSE_PAR | Id.$OPEN_KEY | $BLOCK_SAFE
//					);
//					expect(Id.$SCOLON, $BLOCK_SAFE, Report._1_ERR_MISS_STMT_SCOLON);
//					if (expr != null) {
//						instr.addExpr(expr);
//					}
//				} while (!recover(Id.$CLOSE_PAR, Id.$SCOLON | Id.$OPEN_KEY | $BLOCK_SAFE));
//				expect(Id.$CLOSE_PAR, Id.$SCOLON|Id.$OPEN_KEY|$BLOCK_SAFE, "MSG");
//			}
//			if (expect(Id.$CLOSE_PAR, Id.$CLOSE_PAR|Id.$OPEN_KEY|$BLOCK_SAFE, "MSG")) {
//				return instr;
//		}	}
//		return null;
//	}
