		...
		for (;;) {
			...
			switch (buffer.chars[end]) {
				...
				case 'c': switch (buffer.chars[++end]) {
					...
					case 'o': return keywordOrIdentifier(Id.CONST, Id._CONSTEXPR, /*co*/"nstexpr", 3, 7);
...
//					case 'i': return keywordOrIdentifier(Id._FINAL, /*fi*/"nal", 3);
					case 'o': return keywordOrIdentifier(Id._FOR, /*fo*/"r", 1);
				...
				case 'i': switch (buffer.chars[++end]) {
					...
					case 'l': return keywordOrIdentifier(Id.INLINE, /*inl*/"ine", 3);
						case 't': switch (buffer.chars[++end]) {
							default: if (!isAlphaEx(buffer.chars[end])) return Id._INT;
							case 'e': switch (buffer.chars[++end]) {
								case 'r': switch (buffer.chars[++end]) {
									case 'n': return keywordOrIdentifier(Id.INTERNAL, /*intern*/"al", 2);
									case 'f': return keywordOrIdentifier(Id._INTERFACE, /*interf*/"ace", 3);
						}	}	}		
				}	}	return identifier();
				...
				case 'o': switch (buffer.chars[++end]) {
					case 'p': return keywordOrIdentifier(Id._OPERATOR, /*op*/"erator", 6);
					case 'v': return keywordOrIdentifier(Id.OVERRIDE, /*ov*/"erride", 6);
				}	return identifier();
				...
				case 's': switch (buffer.chars[++end]) {
					...
					case 't': switch (buffer.chars[++end]) {
						...
						case 'r': switch (buffer.chars[++end]) {
							case 'i': return keywordOrIdentifier(Id._STRING, /*stri*/"ng", 2);
							case 'u': return keywordOrIdentifier(Id._STRUCT, /*stru*/"ct", 2);
						}
					}	break;
				...
				case 't': switch (buffer.chars[++end]) {
					...
					case 'y': if (match(/*ty*/"pe", 2)) switch (buffer.chars[++end]) {
				...
				// Preprocessor directives/Annotations
				case '#': 
					if ((buffer.chars[++end]) == '#') {
						end++; 
						return  Id.___ENDIF;
						...
						skipWhitespace();
						switch (buffer.chars[end]) {
						case '#': end++; return Id.___ENDIF;
						case 'd': return ppOrAnnotation(Id.___DEFINE, /*d*/"efine", 5);
						case 'e': switch (buffer.chars[++end]) {
							case 'l': return ppOrAnnotation(Id.___ELSE, /*el*/"se", 2);
							case 'n': return ppOrAnnotation(Id.___ENDIF, /*en*/"dif", 3);
						}
						case 'p': return ppOrAnnotation(Id.___PRAGMA, /*p*/"ragma", 5);
						case 'u': return ppOrAnnotation(Id.___UNDEF, /*u*/"ndef", 4);
						case 'i': switch (buffer.chars[++end]) {
							case 'n': return ppOrAnnotation(Id.___INCLUDE, /*in*/"clude", 5);
							case 'f': switch (buffer.chars[++end]) {
								case 'd': return ppOrAnnotation(Id.___IFDEF, /*ifd*/"ef", 2);
								case 'n': return ppOrAnnotation(Id.___IFNDEF, /*ifn*/"def", 3);
								default: if (!isAlphaEx(buffer.chars[end])) return Id.___IF;
						}	}
...
//					end++;
	...
	private final /*inline*/ long ppOrAnnotation(long keyword, final String s, long len) {
		int i = 0;
		while(buffer.chars[++end] == s.charAt(i++) && i < len);
		// TODO '\0' in buffer ???
		if (i == len && !isAlphaEx(buffer.chars[++end])) {
	...
	private final /*inline*/ long dotOrRealLiteral() {
		...
		if (isNum(c)) {
			...
			return Id._DOT;
		} else if (buffer.chars[++end] == '.') {
			end++;
			return Id._DDD;
		}
		else return Id._SYNTAX_ERR;
	}
	...
	private final /*inline*/ long charLiteral() {
		char cc = buffer.chars[end];
		cc = buffer.chars[++end];
		switch (cc) {
		case '\'': // error
			break;
